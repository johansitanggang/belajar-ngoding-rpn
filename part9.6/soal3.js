...
function countProfit(shoppers) {
  ...
  let resultArrafOfObject = [];
  if (shoppers.length === 0) {
    return resultArrafOfObject;
  } else {
    for (let i = 0; i < listBarang.length; i++) {
      let newObject = {};
      let product = listBarang[i][0];
      let shopperList = [];
      let leftOver = listBarang[i][2];
      let totalProfit = 0;
      for (let j = 0; j < shoppers.length; j++) {
        if (product === shoppers[j].product) {
          if (leftOver >= shoppers[j].amount) {
            shopperList.push(shoppers[j].name);
      ...
      newObject.shopperList = shopperList;
      newObject.leftOver = leftOver;
      newObject.totalProfit = totalProfit;
      resultArrafOfObject.push(newObject);
    }
    return resultArrafOfObject;
  }
}
